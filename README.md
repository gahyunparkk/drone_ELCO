...
[2.](#2-대회-진행-전략) 대회 진행 전략
   ...
   - [2.4.](#24-4단계-링을-통과하기-전-y좌표를-비교하는-과정을-생략) 4단계 링을 통과하기 전 y좌표를 비교하는 과정을 생략
...
- 드론을 색상 마크의 중심에 위치시켰다. 색상 마크의 중심 좌표를 return 하는 square_detect 함수와 드론이 색상 마크 또는 링의 중심에 위치하도록 하는 move_to_center 함수를 이용했다.
...
- 드론이 색상 마크의 중심에 위치하면 색상 마크의 중심, 즉 드론의 위치와 링의 중심을 비교한다. 드론이 링의 중심에 위치하면 다음 동작을 수행한다.[^id]
[^id]: 드론의 위치와 링의 중심의 차이가 100 보다 작은 경우 드론이 링의 중심에 위치했다고 판단한다.
  - 드론이 링의 중심에 위치하지 않는 경우 : 드론을 링의 중심에 위치시켰다.
 
### 2.2. 오차 범위를 증가시키면서 드론을 이동

- 드론을 정확한 곳에 위치시킬 때 드론이 계속해서 상하 또는 좌우로 이동하는 문제가 발생했다.[^id] [^id]:드론의 최소 이동 거리는 20 cm 이기 때문이다.
이 문제를 해결하고, 드론의 이동 시간을 최소화하기 위해 오차 범위에 관한 변수를 설정하고, 변수의 값을 증가시키면서 드론을 이동시켰다.
<br>
<div align=center> 
...
- 드론의 위치와 링 또는 색상 마크의 중심의 차이가 무시할 수 있을 만큼 작으면 드론이 링 또는 색상 마크의 중심에 위치한 것으로 판단했다. 이를 통해 드론이 링 또는 색상 마크의 중심으로 이동하는 데 걸리는 시간을 최소화했다.
- 드론이 상하 또는 좌우로 한 번 이동할 때마다 허용하는 오차 범위를 증가시켰다. 이를 통해 드론이 위치해야 하는 곳에 정확히 위치하기 위해 계속해서 상하 또는 좌우로 이동하는 문제를 해결했다.
