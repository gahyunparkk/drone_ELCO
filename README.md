...
- 드론을 정확한 곳에 위치시킬 때 드론이 계속해서 상하 또는 좌우로 이동하는 문제가 발생했다.[^id] 이 문제를 해결하고, 드론의 이동 시간을 최소화하기 위해 오차 범위에 관한 변수를 설정하고, 변수의 값을 증가시키면서 드론을 이동시켰다.
[^id]:드론의 최소 이동 거리는 20 cm 이기 때문이다.
<br>
<div align=center> 
