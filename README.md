...
4. 텔로 카메라와 링/색상마크의 중심을 잡을 때 조금씩 움직이며 중점을 찾는다. 그러나 인식하고하 하는 물체가 너무 가까이 있으면 미세한 움직임이라도 화면상에서는 좌표 변화가 너무 커 한번 중심을 찾을 때 마다 이동 거리를 줄였다. 그러나 텔로 최소 이동 거리가 0.2m라 대안으로 텔로 카메라의 중심과 찾는 물체의 중심의 오차값을 늘리는 방식을 선택했다.
