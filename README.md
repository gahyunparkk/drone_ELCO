...
- 드론이 이동할 수 있는 최소 거리는 20 cm 이다. 드론이 위치해야 하는 곳에 정확히 위치하기 위해 계속해서 상하 또는 좌우로 이동하는 문제가 발생했다. 위의 문제를 해결하고, 드론이 링 또는 색상 마크의 중심으로 이동하는 데 걸리는 시간을 최소화하기 위해 오차 범위와 관련된 변수를 설정하고, 허용 오차를 증가시키면서 드론을 이동시켰다.
...
- 드론이 이동할 때마다 허용 오차를 증가시켰다. 이를 통해 드론이 위치해야 하는 곳에 정확히 위치하기 위해 계속해서 상하 또는 좌우로 이동하는 문제를 해결했다.
...
- 변수 drone 및 드론 카메라 중심 좌표를 나타내는 변수 center_point 선언
- 변수 cameraObj 선언
- 허용 오차를 나타내는 변수 dif 선언, 40으로 초기화
